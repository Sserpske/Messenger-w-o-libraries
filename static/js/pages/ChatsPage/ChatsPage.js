...
export default class ChatsPage extends Block {
    ...
    getMessagesData(id) {
        ...
        const chat_window = this._element.querySelector('.messenger-app__chat-window');
        if (!chat_window) {
            return;
        }
    ...
    initSelectChatEvent() {
        const select_chat = this._element.querySelectorAll('.js-select-chat');
        select_chat.forEach((element) => {
            element.addEventListener('click', () => {
                const id = Number(element.dataset.chatId);
    ...
    initShowAvailableToAddUsers() {
        const button = this._element.querySelector('.js-add-users');
        if (!button) {
            return;
        }
        const chat_id = button.dataset.chatId;
    ...
    renderAvailableToAddUsers(action) {
    ...
    initAddUsersEvent(chat_id) {
        const add_button = this._element.querySelectorAll('.js-user-add');
    ...
    initShowUsersEvent() {
        const button = this._element.querySelector('.js-show-users');
        if (!button) {
            return;
        }
        const chat_id = button.dataset.chatId;
    ...
    renderUsersList(action, chat_id) {
    ...
    initRemoveUsersEvent(chat_id) {
        const delete_button = this._element.querySelectorAll('.js-user-delete');
    ...
    initUploadChatAvatar() {
        const input = this._element.querySelector('.context-menu__avatar-input');
        if (!input) {
            return;
        }
    ...
    initDeleteChatEvent() {
        const delete_button = this._element.querySelector('.js-delete-chat');
        if (!delete_button) {
            return;
        }
    ...
    initContextMenuEvent() {
        const buttons = this._element.querySelectorAll('.js-toggle-context');
    ...
    initCreateChatEvent() {
        const button = this._element.querySelector('.js-create-chat');
        const field = this._element.querySelector('.messenger-app__new-chat-field');
        const fields_data = {};
        if (!button || !field) {
            return;
        }
    ...
    insertAvatarImage() {
        const avatar = this._element.querySelector('.js-chats-avatar-container');
