...
describe('Тест Button', () => {
  test('Рендер кнопки Создать чат', () => {
    const rendered = new Button({ text: 'Создать чат', button_class: 'messenger-app__new-chat-button js-create-chat' })
      .render();
  ...
  test('Изменения разметки после setProps', () => {
    const button = new Button({ text: 'Создать чат', button_class: 'messenger-app__new-chat-button js-create-chat' });
